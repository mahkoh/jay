...
#[non_exhaustive]
#[derive(Copy, Clone, Debug, Hash, Eq, PartialEq, PrivateEnum)]
pub enum MonoStyle {
...
impl Window {
    ...
    pub fn mono_style(self, target: ContainerTarget) -> MonoStyle {
        get!(MonoStyle::Tabbed)
            .window_mono_style(self, target.to_private())
            .to_public()
    }
    ...
    pub fn set_mono_style(self, style: MonoStyle, target: ContainerTarget) {
        get!().set_window_mono_style(self, style.to_private(), target.to_private())
    }
