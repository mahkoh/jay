...
- `workspace-empty-behavior` (optional):
  ...
  Configures what happens to empty normal workspaces when you leave them or when they become inactive.
...
- `empty-behavior` (optional):

  Overrides `workspace-empty-behavior` for this normal workspace. This has no effect on overlays.
...
Configures what happens to empty normal workspaces when they are left or become inactive.
...
- `hide-on-leave`:
  ...
  Some external workspace tools may still show them as hidden. They can be
  restored by showing the workspace by name. When restoring, the compositor
  prefers the output the workspace was last shown on if it is still connected.
