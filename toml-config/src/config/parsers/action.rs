...
pub struct ActionParser<'a, 'b, 'c>(pub &'a Context<'b, 'c>);

impl ActionParser<'_, '_, '_> {
    fn parse_simple_cmd(&self, span: Span, string: &str) -> ParseResult<Self> {
        ...
        let cmd = match string {
            ...
            "toggle-mono-style" => ToggleMonoStyle,
            "mono-tabbed" => SetMonoStyle(MonoStyle::Tabbed),
            "mono-stacked" => SetMonoStyle(MonoStyle::Stacked),
    ...
    fn parse_exec(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let exec = ext
            .extract(val("exec"))?
    ...
    fn parse_switch_to_vt(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
    ...
    fn parse_show_workspace(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
    ...
    fn parse_show_overlay(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let def = ShowWorkspaceDefaults {
    ...
    fn parse_toggle_overlay(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let def = ShowWorkspaceDefaults {
    ...
    fn parse_move_to_workspace(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let name = ext.extract(str("name"))?.value;
        let ws = self.0.get_workspace_slot(name);
        Ok(Action::MoveToWorkspace { ws })
    }

    fn parse_configure_connector(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let con = ext
    ...
    fn parse_configure_input(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let input = ext
            .extract(val("input"))?
    ...
    fn parse_configure_idle(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let idle = ext
            .extract(val("idle"))?
    ...
    fn parse_configure_output(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let out = ext
            .extract(val("output"))?
    ...
    fn parse_set_env(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let env = ext
            .extract(val("env"))?
    ...
    fn parse_unset_env(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        struct P;
        impl Parser for P {
    ...
    fn parse_set_keymap(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let map = ext
            .extract(val("map"))?
    ...
    fn parse_set_status(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
    ...
    fn parse_set_theme(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let theme = ext
            .extract(val("theme"))?
    ...
    fn parse_set_render_device(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let dev = ext
            .extract(val("dev"))?
    ...
    fn parse_configure_direct_scanout(
        &mut self,
        ext: &mut Extractor<'_, '_, '_>,
    ) -> ParseResult<Self> {
    ...
    fn parse_configure_drm_device(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let dev = ext
            .extract(val("dev"))?
    ...
    fn parse_move_to_output(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
    ...
    fn parse_set_repeat_rate(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let rate = ext
            .extract(val("rate"))?
    ...
    fn parse_undefine_action(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
    ...
    fn parse_named_action(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let (name,) = ext.extract((str("name"),))?;
    ...
    fn parse_create_mark(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
    ...
    fn parse_jump_to_mark(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let (id,) = ext.extract((opt(val("id")),))?;
        let Some(id) = id else {
            return Ok(Action::SimpleCommand {
    ...
    fn parse_push_mode(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let (name,) = ext.extract((str("name"),))?;
        Ok(Action::SetMode {
            name: name.value.to_string(),
            latch: false,
    ...
    fn parse_latch_mode(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let (name,) = ext.extract((str("name"),))?;
        Ok(Action::SetMode {
            name: name.value.to_string(),
            latch: true,
    ...
    fn parse_create_virtual_output(
        &mut self,
        ext: &mut Extractor<'_, '_, '_>,
    ) -> ParseResult<Self> {
        let (name,) = ext.extract((str("name"),))?;
    ...
    fn parse_remove_virtual_output(
        &mut self,
        ext: &mut Extractor<'_, '_, '_>,
    ) -> ParseResult<Self> {
        let (name,) = ext.extract((str("name"),))?;
    ...
    fn parse_resize(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
    ...
    fn parse_hide_overlay(&mut self, ext: &mut Extractor<'_, '_, '_>) -> ParseResult<Self> {
        let (name,) = ext.extract((str("name"),))?;
        let ws = self.0.get_workspace_slot(name.value);
...
impl Parser for ActionParser<'_, '_, '_> {
    type Value = Action;
    type Error = ActionParserError;
