...
use {
    ...
    jay_config::{
        ...
        set_show_float_pin_icon, set_show_title_separator, set_ui_drag_enabled,
        ...
        toggle_float_above_fullscreen, toggle_show_bar, toggle_show_title_separator,
        video::{
    ...
    fn into_fn_impl<B: FnBuilder>(self, b: &B, state: &Rc<State>) -> B::Output {
        ...
        match self {
            Action::SimpleCommand { cmd } => match cmd {
                ...
                SimpleCommand::ShowTitleSeparator(show) => {
                    b.new(move || set_show_title_separator(show))
                }
                SimpleCommand::ToggleTitleSeprator => b.new(toggle_show_title_separator),
...
fn load_config(initial_load: bool, auto_reload: bool, persistent: &Rc<PersistentState>) {
    ...
    *state.persistent.client_rule_mapper.borrow_mut() = Some(client_rule_mapper);
    ...
    if let Some(v) = config.show_title_separator {
        set_show_title_separator(v);
    }
