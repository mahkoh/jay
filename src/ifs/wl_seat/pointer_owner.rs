...
impl PointerOwnerHolder {
    ...
    pub fn workspace_changed(
        ...
        self.owner.get().workspace_changed(seat, on, ws, reason);
    }
}
...
impl<T: SimplePointerOwnerUsecase> PointerOwner for SimplePointerOwner<T> {
    fn button(&self, seat: &Rc<WlSeatGlobal>, time_usec: u64, button: u32, state: ButtonState) {
        ...
        seat.pointer_owner.workspace_changed(
            seat,
            &on,
            on.workspace().as_ref(),
            WorkspaceChangeReason::InitialGrab,
        );
    }
...
impl<T: SimplePointerOwnerUsecase> PointerOwner for SimpleGrabPointerOwner<T> {
    ...
    fn workspace_changed(
        ...
        self.usecase.workspace_changed(self, seat, on, ws, reason);
    }
}
...
impl SimplePointerOwnerUsecase for DefaultPointerUsecase {
    ...
    fn workspace_changed(
        ...
        grab.node
            .clone()
            .node_grab_workspace_changed(tt, seat, on, ws, reason);
    }
}
...
impl WindowManagementGrabUsecase for ResizeToplevelGrabPointerOwner {
    ...
    fn apply_changes(
        ...
        if x1.is_some() || x2.is_some() || y1.is_some() || y2.is_some() {
            let tt = &seat.state.tree_transaction();
            parent.cnode_resize_child(tt, &**tl, x1, y1, x2, y2);
        }
    }
}
    ...
    fn left_button_up(&self, seat: &Rc<WlSeatGlobal>) {
        ...
        match dest {
            ...
            TddType::NewWorkspace { output } => {
                new_container(&output.ensure_normal_workspace(tt));
            }
            ...
            TddType::MoveToNewWorkspace { output } => {
                let ws = output.generate_normal_workspace(tt);
                src_parent.cnode_remove_child(tt, &*src);
