...
impl ZwlrGammaControlV1 {
    ...
    fn detach(&self) {
        if let Some(node) = self.output.node() {
            if let Some(active_zwlr_gamma_control) = node.active_zwlr_gamma_control.get() {
                if active_zwlr_gamma_control.id() == self.id() {
                    node.active_zwlr_gamma_control.set(None);
                    _ = node.set_gamma_lut(None);
                }
            }
        }
    }
...
impl ZwlrGammaControlV1RequestHandler for ZwlrGammaControlV1 {
    ...
    fn set_gamma(&self, req: SetGamma, _slf: &Rc<Self>) -> Result<(), Self::Error> {
        ...
        if !node.set_gamma_lut(Some(Rc::new(BackendGammaLut::new(gamma_lut)))) {
            node.active_zwlr_gamma_control.set(None);
            _ = node.set_gamma_lut(None);
            self.send_failed();
        }
