use {
    crate::{
        ...
        object::Object,
        rect::Region,
        tree::{self, Node, OutputNode, TreeTimeline::LiveTL},
...
impl ExtImageCopyCaptureFrameV1 {
    ...
    pub(super) fn copy_texture(
        ...
        let transform = on.node_state[LiveTL].transform.get();
    ...
    pub(super) fn copy_node(self: &Rc<Self>, on: &OutputNode, node: &dyn Node, size: (i32, i32)) {
        let scale = on.node_state[LiveTL].scale.get();
        self.copy(on, size, |fb, aq, re| {
            fb.render_node(
                ...
                node,
                &self.client.state,
                Some(node.node_absolute_position(LiveTL)),
                scale,
                true,
                true,
                true,
