...
async fn test(run: Rc<TestRun>) -> TestResult {
    ...
    let fifo = client.get_fifo(surface);
    client.sync().await;
    ...
    fifo.set_barrier();
    let buffer1 = client.create_single_pixel_buffer(Color::from_srgb(255, 0, 0));
    surface.attach(buffer1.id);
    surface.commit();
    client.sync().await;

    // Test 1a: Critical test - without wait_barrier, commit should be applied immediately
    let buffer1a = client.create_single_pixel_buffer(Color::from_srgb(255, 128, 0));
    ...
    let buffer2_current = client.create_single_pixel_buffer(Color::from_srgb(0, 255, 0));
    let buffer2_next = client.create_single_pixel_buffer(Color::from_srgb(0, 0, 255));
    ...
    let buffer3_current = client.create_single_pixel_buffer(Color::from_srgb(128, 128, 0));
    let buffer3_next = client.create_single_pixel_buffer(Color::from_srgb(0, 128, 128));
