...
use crate::it::test_utils::test_output_setup::create_output;
use crate::it::test_utils::test_output_setup::get_output;
use crate::it::test_utils::test_output_setup::wait_for_output_addition;
use crate::it::test_utils::test_output_setup::wait_for_output_removal;
use crate::it::testrun::DefaultSetup;
...
async fn test(run: Rc<TestRun>) -> TestResult {
    ...
    test_toggle_floating_keeps_mapping_behavior(&run, &ds).await?;
...
async fn test_toggle_floating_keeps_mapping_behavior(
    run: &Rc<TestRun>,
    ds: &DefaultSetup,
) -> TestResult {
    run.cfg
        .set_workspace_empty_behavior(WorkspaceEmptyBehavior::Hide)?;
    run.cfg.show_workspace(ds.seat.id(), "toggle-floating")?;
    let workspace = run.cfg.get_workspace("toggle-floating")?;
    let client = run.create_client().await?;
    let win = client.create_window().await?;
    win.map().await?;
    let window = run.cfg.get_workspace_window("toggle-floating")?;
    run.cfg.show_workspace(ds.seat.id(), "toggle-active")?;
    run.cfg.set_window_floating(window, true)?;
    tassert!(run.cfg.get_workspaces()?.contains(&workspace));
    run.cfg.set_window_floating(window, false)?;
    tassert!(!run.cfg.get_workspaces()?.contains(&workspace));
    tassert_ne!(run.cfg.get_workspace_window("toggle-active")?, Window(0));
    Ok(())
}
