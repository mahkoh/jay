...
pub enum VulkanError {
    ...
    BusyInTransfer,
    #[error("Driver does not support dynamic rendering local read")]
    NoDynamicRenderingLocalRead,
    #[error("Driver does not support descriptor buffers")]
    NoDescriptorBuffer,
    #[error("Driver does not support independent blend")]
    NoIndependentBlend,
}
...
impl GfxContext for Context {
    ...
    fn create_blend_buffer(
        ...
        let buffer = self.0.create_blend_buffer(width, height)?;
        Ok(buffer)
    }
}
