...
impl VulkanRenderer {
    pub fn create_blend_buffer(
        self: &Rc<Self>,
        width: i32,
        height: i32,
    ) -> Result<Rc<VulkanImage>, VulkanError> {
        if self.device.dynamic_rendering_local_read.is_none() {
            return Err(VulkanError::NoDynamicRenderingLocalRead);
        }
        ...
        let input_attachment_descriptor = {
            let mut buf = vec![0; self.device.input_attachment_descriptor_size].into_boxed_slice();
