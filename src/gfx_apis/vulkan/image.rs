...
impl VulkanShmImage {
    pub fn upload(&self, buffer: &[Cell<u8>]) -> Result<(), VulkanError> {
        let buffer = buffer.iter().map(|b| b.get()).collect();
...
impl VulkanDmaBufImageTemplate {
    ...
    fn create_image(
        ...
        let image = {
            ...
            let usage = match (for_rendering, shm.is_some()) {
        ...
        res.map_err(VulkanError::BindImageMemory)?;
        let view = device.create_image_view(image, self.format)?;
