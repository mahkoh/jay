use {
    ...
    arrayvec::ArrayVec,
    ash::{
        extensions::khr::{ExternalMemoryFd, ExternalSemaphoreFd, PushDescriptor},
        vk::{
            ...
            ExternalSemaphoreProperties, KhrDriverPropertiesFn, KhrExternalMemoryFdFn,
            KhrExternalSemaphoreFdFn, KhrPushDescriptorFn, MemoryPropertyFlags, MemoryType,
            PhysicalDevice, PhysicalDeviceDriverPropertiesKHR, PhysicalDeviceDrmPropertiesEXT,
...
impl VulkanDevice {
    ...
    // pub(super) fn create_tex_descriptor_set(&self) -> Result<Rc<VulkanDescriptorSet>, VulkanError> {
    //     let mut pools = self.tex_descriptor_pools.borrow_mut();
    //     for pool in &*pools {
    //         if let Some(set) = pool.allocate_descriptor_set()? {
    //             return Ok(set);
    //         }
    //     }
    //     let new = self.create_descriptor_pool(&self.tex_descriptor_set_layout)?;
    //     let set = match new.allocate_descriptor_set()? {
    //         None => return Err(VulkanError::DescriptorSetPoolOom),
    //         Some(set) => set,
    //     };
    //     pools.push(new);
    //     Ok(set)
...
impl VulkanInstance {
    ...
    fn find_dev(&self, drm: &Drm) -> Result<PhysicalDevice, VulkanError> {
        ...
        log::info!(
            "Searching for vulkan device with device id {}:{}",
            uapi::major(dev),
            uapi::minor(dev)
        );
        ...
        for phy_dev in phy_devs {
            ...
            let should_log = self.logged_devices.set(props.device_id, ()).is_none();
            if should_log {
                log::info!("Found new physical device:");
            ...
            if props.api_version < API_VERSION {
                if should_log {
                    log::warn!(
                        "Device with id {} does not support vulkan 1.3",
                        props.device_id
                    );
                }
                continue;
            ...
            if !extensions.contains_key(ExtPhysicalDeviceDrmFn::name()) {
                if should_log {
                    log::warn!(
                        "Device with id {} does support not the VK_EXT_physical_device_drm extension",
                        props.device_id
                    );
                }
                continue;
