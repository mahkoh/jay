use {
    crate::{
        control_center::{
            CcBehavior, ControlCenterInner, EnumText, PaneType,
        ...
        state::State,
        tree::{ToplevelData, ToplevelNode, ToplevelType},
        utils::{copyhashmap::CopyHashMap, toplevel_identifier::ToplevelIdentifier},
    },
    egui::{CollapsingHeader, Sense, TextFormat, Ui, Widget, text::LayoutJob},
    egui_material_icons::icons::ICON_OPEN_IN_NEW,
    isnt::std_1::primitive::IsntStrExt,
    jay_config::window::{ContentType, GAME_CONTENT, PHOTO_CONTENT, VIDEO_CONTENT},
    linearize::Linearize,
    std::rc::{Rc, Weak},
};
...
impl EnumText for WindowCritTy {
...
pub struct WindowPane {
    ...
    _leaf: Rc<CritLeafMatcher<ToplevelData>>,
}
...
impl ControlCenterInner {
    pub fn create_window_pane(self: &Rc<Self>, window: &Rc<dyn ToplevelNode>) -> WindowPane {
        ...
        let node = state
            .tl_matcher_manager
            .identifier(window.tl_data().identifier.get());
        let cc = Rc::downgrade(self);
        let leaf = state.tl_matcher_manager.leaf(&node, move |_| {
            if let Some(cc) = cc.upgrade() {
...
pub fn show_window_collapsible(
    ...
    CollapsingHeader::new(layout_job)
        .id_salt(("window", window.tl_data().identifier.get()))
        .show(ui, |ui| {
