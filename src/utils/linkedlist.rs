...
impl<T> LinkedList<T> {
    ...
    pub fn append_all(&self, other: &LinkedList<T>) {
        ...
        unsafe {
            ...
            let o_first = o_root.deref().next.get();
            let o_last = o_root.deref().prev.get();
            let s_first = self.root.data;
            let s_last = s_first.deref().prev.get();
            o_first.deref().prev.set(s_last);
            s_last.deref().next.set(o_first);
            o_last.deref().next.set(s_first);
            s_first.deref().prev.set(o_last);
            o_root.deref().next.set(o_root);
            o_root.deref().prev.set(o_root);
        }
    }
    ...
    pub fn last(&self) -> Option<NodeRef<T>> {
        unsafe { self.endpoint(self.root.data.deref().prev.get()) }
    }

    pub fn first(&self) -> Option<NodeRef<T>> {
        unsafe { self.endpoint(self.root.data.deref().next.get()) }
    }
    ...
    pub fn rotate_last(&self, t: &NodeRef<T>) {
        unsafe {
            let root = self.root.data.deref();
            root.prev.get().deref().next.set(root.next.get());
            root.next.get().deref().prev.set(root.prev.get());
            root.prev.set(t.data);
            root.next.set(t.data.deref().next.get());
            t.data.deref().next.get().deref().prev.set(self.root.data);
            t.data.deref().next.set(self.root.data);
        }
    }
    ...
    pub fn iter(&self) -> LinkedListIter<T> {
        unsafe {
            let root = self.root.data.deref();
            root.rc.fetch_add(1);
            root.next.get().deref().rc.fetch_add(1);
            LinkedListIter {
                root: self.root.data,
                next: root.next.get(),
            }
    ...
    pub fn rev_iter(&self) -> RevLinkedListIter<T> {
        unsafe {
            let root = self.root.data.deref();
            root.rc.fetch_add(1);
            root.prev.get().deref().rc.fetch_add(1);
            RevLinkedListIter {
                root: self.root.data,
                next: root.prev.get(),
            }
...
impl<T> Iterator for LinkedListIter<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        unsafe {
            ...
            self.next = old_next.deref().next.get();
            self.next.deref().rc.fetch_add(1);
...
impl<T> Iterator for RevLinkedListIter<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        unsafe {
            let old_next = self.next;
            self.next = old_next.deref().prev.get();
            self.next.deref().rc.fetch_add(1);
            Some(NodeRef { data: old_next })
        }
    }
...
impl<T: Debug> Debug for LinkedNode<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        unsafe { self.data.deref().data.as_ref().unwrap_unchecked().fmt(f) }
    }
}
...
impl<T> Deref for NodeRef<T> {
    ...
    fn deref(&self) -> &Self::Target {
        unsafe { self.data.deref().data.as_ref().unwrap_unchecked() }
    }
}
...
impl<T> Clone for NodeRef<T> {
    fn clone(&self) -> Self {
        unsafe {
            self.data.deref().rc.fetch_add(1);
            Self { data: self.data }
        }
    }
}
...
impl<T> NodeRef<T> {
        ...
        F: FnOnce(&NodeData<T>) -> &Cell<NonNull<NodeData<T>>>,
    {
        unsafe {
            ...
            if other.deref().data.is_some() {
                other.deref().rc.fetch_add(1);
    ...
    pub fn detach(&self) {
        unsafe {
            let data = self.data.deref();
            data.prev.get().deref().next.set(data.next.get());
            data.next.get().deref().prev.set(data.prev.get());
...
unsafe fn prepend_existing<T>(data: NonNull<NodeData<T>>, t: &NodeRef<T>) {
    unsafe {
        ...
        tref.next.set(data);
        dref.prev.get().deref().next.set(t.data);
...
unsafe fn append_existing<T>(data: NonNull<NodeData<T>>, t: &NodeRef<T>) {
    unsafe {
        ...
        dref.next.get().deref().prev.set(t.data);
