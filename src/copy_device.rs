use {
    crate::{
        ...
        vulkan_core::{VulkanCoreError, VulkanCoreInstance},
    },
    ahash::AHashSet,
    arrayvec::ArrayVec,
    ash::{
        ...
        vk::{
            self, API_VERSION_1_3, AccessFlags2, BindImageMemoryInfo, BindImagePlaneMemoryInfo,
            BlitImageInfo2, BufferCopy2, BufferCreateInfo, BufferImageCopy2, BufferMemoryBarrier2,
            ...
            Extent3D, ExternalBufferProperties, ExternalFenceHandleTypeFlags,
            ...
            PhysicalDeviceExternalBufferInfo, PhysicalDeviceExternalImageFormatInfoKHR,
            ...
            PhysicalDeviceProperties2, PhysicalDeviceSynchronization2Features, PipelineStageFlags2,
            QUEUE_FAMILY_FOREIGN_EXT, Queue, QueueFlags, SampleCountFlags, SemaphoreCreateInfo,
            SemaphoreImportFlags, SemaphoreSubmitInfo, SharingMode, SubmitInfo2, SubresourceLayout,
            WHOLE_SIZE,
        },
    },
...
pub enum CopyDeviceError {
    ...
    NoSynchronization2,
    #[error("Device does not support the external semaphore fd extension")]
    NoExternalSemaphoreFd,
    #[error("Device does not support the external fence fd extension")]
    NoExternalFenceFd,
    #[error("Device does not support the external memory fd extension")]
    NoExternalMemoryFd,
    #[error("Device does not support the external memory dma buf extension")]
    NoExternalMemoryDmaBuf,
    #[error("Device does not support the image drm format modifier extension")]
    NoImageDrmFormatModifier,
    #[error("Device does not support the foreign queue extension")]
    NoQueueFamilyForeign,
    #[error("Device does not support sync files")]
    NoSyncFiles,
    #[error("Device does not importing dma bufs as buffers")]
    NoDmaBufBufferImport,
    #[error("Device does not have any useful queue families")]
    NoQueueFamily,
...
pub struct PhysicalCopyDevice {
    ...
    support: StaticMap<Dir, Vec<CopyDeviceSupport>>,
    queues_to_allocate: Vec<QueueToAllocate>,
    queues: Keyed<QueueIndex>,
...
enum CopyDeviceCopyType {
    BufferToBuffer {
        regions: RefCell<Vec<BufferCopy2<'static>>>,
        src: ImportedBuffer,
        dst: ImportedBuffer,
        stride: u32,
        bpp: u32,
    ...
    BufferToImage {
        regions: RefCell<Vec<BufferImageCopy2<'static>>>,
        buf: ImportedBuffer,
    ...
    ImageToImage {
        regions: RefCell<Vec<ImageCopy2<'static>>>,
        src: ImportedImage,
        dst: ImportedImage,
    },
    Blit {
        regions: RefCell<Vec<ImageBlit2<'static>>>,
        src: ImportedImage,
        dst: ImportedImage,
    },
}
...
impl PhysicalCopyDevice {
    pub fn new(
        ...
        let instance = VulkanCoreInstance::new(Level::Debug)?;
        let physical_device;
        ...
        'find_device: {
            ...
            'outer: for phy in devices {
                let res = unsafe { instance.instance.enumerate_device_extension_properties(phy) };
                let exts = match res {
                    Ok(res) => res,
                    Err(e) => {
                        log::error!(
                ...
                unsafe {
                    instance
                        .instance
                        .get_physical_device_properties2(phy, &mut props);
                }
        ...
        if device_properties.api_version < API_VERSION {
        ...
        }
        {
            ...
            let mut supports_external_semaphore_fd = false;
            let mut supports_external_fence_fd = false;
            let mut supports_external_memory_fd = false;
            let mut supports_external_memory_dma_buf = false;
            let mut supports_image_drm_format_modifier = false;
            let mut supports_queue_family_foreign = false;
            for ext in &device_extensions {
                let Ok(name) = ext.extension_name_as_c_str() else {
                    continue;
                };
                supports_external_semaphore_fd |= name == external_semaphore_fd::NAME;
                supports_external_fence_fd |= name == external_fence_fd::NAME;
                supports_external_memory_fd |= name == external_memory_fd::NAME;
                supports_external_memory_dma_buf |= name == external_memory_dma_buf::NAME;
                supports_image_drm_format_modifier |= name == image_drm_format_modifier::NAME;
                supports_queue_family_foreign |= name == queue_family_foreign::NAME;
            }
            if !supports_external_semaphore_fd {
                return Err(CopyDeviceError::NoExternalSemaphoreFd);
            }
            if !supports_external_fence_fd {
                return Err(CopyDeviceError::NoExternalFenceFd);
            }
            if !supports_external_memory_fd {
                return Err(CopyDeviceError::NoExternalMemoryFd);
            }
            if !supports_external_memory_dma_buf {
                return Err(CopyDeviceError::NoExternalMemoryDmaBuf);
            }
            if !supports_image_drm_format_modifier {
                return Err(CopyDeviceError::NoImageDrmFormatModifier);
            }
            if !supports_queue_family_foreign {
                return Err(CopyDeviceError::NoQueueFamilyForeign);
            }
        }
        {
            ...
            let supported = props.external_semaphore_features.contains(
                ExternalSemaphoreFeatureFlags::EXPORTABLE
                    | ExternalSemaphoreFeatureFlags::IMPORTABLE,
            );
            if !supported {
                return Err(CopyDeviceError::NoSyncFiles);
            }
        }
        {
            ...
            supports_dmabuf_export = props
                .external_memory_properties
                .external_memory_features
                .contains(ExternalMemoryFeatureFlags::EXPORTABLE);
        }
        let queues_to_allocate;
        let queue_indices;
        {
            let families = unsafe {
                instance
            ...
            let gfx = gfx.ok_or(CopyDeviceError::NoQueueFamily)?;
            (queues_to_allocate, queue_indices) = allocate_queues(gfx, compute_only, transfer_only);
        }
        let mut support = StaticMap::<_, Vec<_>>::default();
        for format in FORMATS {
            ...
            let mut modifiers = DrmFormatModifierPropertiesListEXT::default()
                .drm_format_modifier_properties(&mut list);
            let mut out = FormatProperties2::default().push_next(&mut modifiers);
            unsafe {
                instance.instance.get_physical_device_format_properties2(
                    physical_device,
            ...
            for modifier in list {
                for direction in Dir::variants() {
                    ...
                    let supports_blit = modifier
                        .drm_format_modifier_tiling_features
                        .contains(blit_feature_flags);
                    ...
                    let supported = external_memory_prop
                        .external_memory_properties
                        .external_memory_features
                        .contains(ExternalMemoryFeatureFlags::IMPORTABLE);
                    if !supported {
                    ...
                    if me.width > 0 && me.height > 0 && me.depth > 0 {
                        support[direction].push(CopyDeviceSupport {
                            format,
    ...
    pub fn create_device(self: &Rc<Self>) -> Result<Rc<CopyDevice>, CopyDeviceError> {
        let device = {
            ...
            let queue_create_info: Vec<_> = self
                ...
                .collect();
            let extensions = [
                external_semaphore_fd::NAME.as_ptr(),
                external_fence_fd::NAME.as_ptr(),
                external_memory_fd::NAME.as_ptr(),
                external_memory_dma_buf::NAME.as_ptr(),
                image_drm_format_modifier::NAME.as_ptr(),
                queue_family_foreign::NAME.as_ptr(),
            ];
        ...
        let external_semaphore_fd =
            external_semaphore_fd::Device::new(&self.instance.instance, &device);
        let external_fence_fd = external_fence_fd::Device::new(&self.instance.instance, &device);
        let external_memory_fd = external_memory_fd::Device::new(&self.instance.instance, &device);
        let queues = self
            .queues
            .each_ref()
            .map_values(|idx| unsafe { device.get_device_queue(idx.family, idx.idx_within_family) })
            .into_static_map();
        ...
        let mut submissions_list = vec![];
        for queue in &self.queues_to_allocate {
            let mut pending_per_queue_list = vec![];
            for _ in 0..queue.num {
                let pending_submissions = Rc::new(PendingSubmissions {
                    task_has_pending: Default::default(),
                    pending: Default::default(),
                });
                pending_per_queue_list.push(pending_submissions.clone());
            }
            submissions_list.push(pending_per_queue_list);
        }
        ...
        let mut tasks = vec![];
        for submissions in submissions_list.iter().flatten() {
            let submissions = submissions.clone();
            let dev = dev.clone();
            let ring = self.ring.clone();
            let task = self.eng.spawn("copy-device-await-pending", async move {
...
impl CopyDevice {
    ...
    fn classify_dmabuf(
        ...
        let Some(format) = self.dev.phy.support[dir]
            .iter()
            .find(|s| (s.format, s.modifier) == (buf.format, buf.modifier))
        else {
        ...
        let buffer_possible = buf.modifier == LINEAR_MODIFIER
            && buf.planes.len() == 1
            && buf.planes[0].stride % format.format.bpp == 0
            && width <= buf.planes[0].stride / format.format.bpp;
        Ok(ClassifiedDmabuf {
            fd_props,
            on_device,
            buffer_possible,
    ...
    fn import_buffer(
        ...
        buf: &DmaBuf,
        dir: Dir,
    ) -> Result<ImportedBuffer, CopyDeviceError> {
    ...
    fn import_image(
        ...
        buf: &DmaBuf,
        dir: Dir,
    ) -> Result<ImportedImage, CopyDeviceError> {
        ...
        let destroy_image = on_drop(|| unsafe { self.dev.dev.destroy_image(image, None) });
        ...
        let mut free_memories = PlaneVec::new();
        {
            ...
            for plane_idx in 0..num_device_memories {
                ...
                unsafe {
                    self.dev.dev.get_image_memory_requirements2(
                ...
                free_memories.push(on_drop(move || unsafe {
                    self.dev.dev.free_memory(device_memory, None)
                }));
            }
    ...
    pub fn create_copy(
        ...
        let tt = match (src_class.on_device, dst_class.on_device) {
            ...
            (false, true) => TransferType::Download,
            (true, false) => TransferType::Upload,
        ...
        let free_command_buffer = on_drop(|| unsafe {
            self.dev
                .dev
                .free_command_buffers(self.dev.pools[tt], &[command_buffer])
        });
        let ty = if blit {
...
impl CopyDeviceCopy {
    pub fn execute(
        ...
        let queue_family = self.inner.dev.phy.queues[tt].family;
        let region_buf;
        let region = match region {
            Some(r) => r,
            _ => {
                region_buf = Region::new(Rect::new_saturating(
                    0,
                    0,
                    slf.width as i32,
                    slf.height as i32,
        ...
        let (x_mask, y_mask) = self.inner.dev.phy.queues[tt].transfer_granularity_mask;
        let rects = &mut *slf.rects.borrow_mut();
        rects.clear();
        for rect in region.iter() {
            let x1 = (rect.x1().max(0) as u32 & !x_mask).min(width);
            let y1 = (rect.y1().max(0) as u32 & !y_mask).min(height);
            let x2 = ((rect.x2().max(0) as u32).saturating_add(x_mask) & !x_mask).min(width);
            let y2 = ((rect.y2().max(0) as u32).saturating_add(y_mask) & !y_mask).min(height);
            let width = x2 - x1;
            let height = y2 - y1;
            if width == 0 || height == 0 {
                continue;
        ...
        macro_rules! initial_buffer_barrier {
        ...
        macro_rules! final_buffer_barrier {
            ($buf:expr, $access:expr) => {
                BufferMemoryBarrier2::default()
        ...
        macro_rules! initial_image_barrier {
        ...
        macro_rules! final_image_barrier {
            ($img:expr, $layout:expr, $access:expr) => {
                ImageMemoryBarrier2::default()
                    .src_stage_mask(PipelineStageFlags2::TRANSFER)
                    .src_access_mask($access)
        ...
        match &slf.ty {
            CopyDeviceCopyType::BufferToBuffer {
                ...
                let regions = &mut *regions.borrow_mut();
                regions.clear();
                ...
                let initial_barriers = [
                    initial_buffer_barrier!(src, A::TRANSFER_READ),
                    initial_buffer_barrier!(dst, A::TRANSFER_WRITE),
                ];
                let final_barriers = [
                    final_buffer_barrier!(src, A::TRANSFER_READ),
                    final_buffer_barrier!(dst, A::TRANSFER_WRITE),
                ];
            ...
            | CopyDeviceCopyType::ImageToBuffer {
                ...
                for &mut (x, y, width, height) in rects {
                    ...
                    let region = BufferImageCopy2::default()
                        ...
                        .buffer_image_height(self.inner.height)
                ...
                let initial_image_barriers =
                    [initial_image_barrier!(img, image_layout, image_access_mask)];
                let final_image_barriers =
                    [final_image_barrier!(img, image_layout, image_access_mask)];
                let initial_buffer_barriers = [initial_buffer_barrier!(buf, buffer_access_mask)];
                let final_buffer_barriers = [final_buffer_barrier!(buf, buffer_access_mask)];
            ...
            CopyDeviceCopyType::ImageToImage { regions, src, dst } => {
                let regions = &mut *regions.borrow_mut();
                regions.clear();
                for &mut (x, y, width, height) in rects {
                ...
                let initial_barriers = [
                    initial_image_barrier!(src, L::TRANSFER_SRC_OPTIMAL, A::TRANSFER_READ),
                    initial_image_barrier!(dst, L::TRANSFER_DST_OPTIMAL, A::TRANSFER_WRITE),
                ];
                let final_barriers = [
                    final_image_barrier!(src, L::TRANSFER_SRC_OPTIMAL, A::TRANSFER_READ),
                    final_image_barrier!(dst, L::TRANSFER_DST_OPTIMAL, A::TRANSFER_WRITE),
                ];
                let initial_dependency_info =
            ...
            CopyDeviceCopyType::Blit { regions, src, dst } => {
                let regions = &mut *regions.borrow_mut();
                regions.clear();
                for &mut (x, y, width, height) in rects {
                    let x1 = x;
        ...
        if let Some(sync_file) = sync_file {
            let semaphore = match self.inner.dev.semaphores.pop() {
                Some(s) => s,
                _ => self.inner.dev.create_semaphore()?,
            };
        ...
        let signal_fence = match self.inner.dev.fences.pop() {
            Some(s) => s,
            _ => self.inner.dev.create_fence()?,
        };
        ...
        let sync_file = match signal_fence.export() {
            ...
            Err(e) => {
                ...
                self.inner.dev.wait_idle();
                None
            }
        };
        self.inner.busy.set(sync_file.clone());
        let pending = Pending {
        ...
        self.inner.dev.submissions[tt].pending.push(pending);
        Ok(sync_file)
    }
}
...
impl Drop for CopyDevice {
    fn drop(&mut self) {
        ...
        self.dev.semaphores.take();
        self.dev.fences.take();
    }
}
...
) -> (Vec<QueueToAllocate>, Keyed<QueueIndex>) {
