...
pub struct OutputNode {
    ...
    pub tray_items: LinkedList<Rc<ExtTrayItemV1>>,
}
...
impl OutputNode {
    ...
    pub fn update_tray_positions(self: &Rc<Self>) {
        ...
        for item in self.tray_items.rev_iter() {
            if item.surface.buffer.is_none() {
                continue;
            }
            have_any = true;
            right -= th;
            let rel_pos =
                Rect::new_sized(right, 1, th.saturating_sub(2), th.saturating_sub(2)).unwrap();
                ...
                if y < bar_height {
                    ...
                    for item in self.tray_items.iter() {
                        ...
                        if pos.contains(x, y) {
                            ...
                            return item.node_find_tree_at(x, y, tree, usecase);
                        }
                    }
                } else {
                    if let Some(ws) = self.workspace.get() {
