...
impl ContainerChildInner {
    fn new(
        state: &Rc<State>,
        container_theme: &ContainerTheme,
        node: &Rc<dyn ToplevelNode>,
        factor: f64,
        resize_handle: Option<Rect>,
        self_tl_data: Option<&ToplevelData>,
    ) -> Self {
        let slf = Self {
            id: node.node_id(),
            node: node.clone(),
        ...
        let theme =
            compute_child_theme(&state.theme, container_theme, self_tl_data, node.tl_data());
...
impl ContainerNode {
    ...
    fn perform_split_layout(self: &Rc<Self>) {
        ...
        let theme = &ns.theme;
        let border_width = theme.sizes.border_width.get();
        let title_height_tmp = theme.sizes.title_height.get();
        let title_plus_underline_height = theme.sizes.title_plus_underline_height.get();
    ...
    fn render_titles(self: &Rc<Self>) -> Rc<AsyncEvent> {
        ...
        for child in self.children.iter_valid(RenderTL) {
            ...
            let color = match cns.ty.get() {
                ...
                ContainerChildType::Other => colors.unfocused_title_text.get(),
            };
    ...
    fn compute_render_positions(self: &Rc<Self>) {
        ...
        let mut prev_focused_border = Color::TRANSPARENT;
        let mut add_border = |rd: &mut ContainerRenderData,
            ...
            let focused_border = child.node_state[RenderTL].theme.colors.focused_border.get();
            let border_color = if fill_active_borders && active {
                focused_border
            } else if fill_active_borders && prev_active {
                prev_focused_border
            } else {
            ...
            prev_focused_border = focused_border;
        };
        ...
        if cb == ContainerBorders::Full {
            ...
            } else if let Some(child) = self.children.last_valid(RenderTL) {
                let (x, y) = match split {
                    ContainerSplit::Horizontal => (fwidth, sp),
                    ContainerSplit::Vertical => (sp, fheight),
                };
                add_border(rd, &child, x, y, false, prev_active, true);
    ...
    fn tile_drag_destination_mono_titles(
        ...
        let rect = Rect::new(
            ...
            0,
            ns.width.get(),
            ns.theme.sizes.title_height.get(),
        )?
        .move_(ns.abs_x1.get(), ns.abs_y1.get())
        .intersect(abs_bounds);
        if rect.contains(abs_x, abs_y) {
    ...
    pub fn container_borders(&self, timeline: TreeTimeline) -> ContainerBorders {
        let ns = &self.node_state[timeline];
        match ns.theme.container_borders.get() {
    ...
    fn compute_theme(&self) -> ContainerTheme {
        compute_theme(&self.state.theme, Some(&self.toplevel_data))
    }

    fn compute_child_theme(&self, child: &ContainerChild) -> ContainerChildTheme {
        compute_child_theme(
            &self.state.theme,
            &self.node_state[LiveTL].theme,
            Some(&self.toplevel_data),
            child.node.tl_data(),
        )
    }
    ...
    fn child_theme_changed(
        ...
        let theme = self.compute_child_theme(child);
        let changed = ns.theme.cached_update(theme, |op| {
            self.add_child_op(child, ContainerChildTransactionOp::ThemeOp(op));
        });
        let ContainerChildThemeChanged {
            colors:
                ContainerChildThemeColorsChanged {
                    focused_title_text,
    ...
    pub fn get_child_node(&self, node_id: NodeId) -> Option<NodeRef<ContainerChild>> {
        self.child_nodes.borrow().get(&node_id).map(|c| c.to_ref())
    }
}
...
impl ContainingNode for ContainerNode {
    ...
    fn cnode_set_child_position(self: Rc<Self>, child: &dyn Node, x: i32, y: i32) {
        ...
        let ns = &self.node_state[LiveTL];
        let tpuh = ns.theme.sizes.title_plus_underline_height.get();
        if ns.mono_child.is_some() {
    ...
    fn cnode_child_theme_changed(self: Rc<Self>, child: NodeId) {
        ...
        let Some(child) = child_nodes.get(&child) else {
            return;
        };
        self.child_theme_changed(false, false, false, child);
    }
}
...
impl ThemeChangeListener for ContainerNode {
    fn changed(self: Rc<Self>) {
        let ns = &self.node_state[LiveTL];
        let changed = ns.theme.cached_update(self.compute_theme(), |op| {
            self.add_transaction_op(ContainerTransactionOp::ThemeOp(op));
        });
        let ContainerThemeChanged {
            container_borders,
            colors:
                ContainerThemeColorsChanged {
                    border, //
                    separator,
                },
            sizes:
                ContainerThemeSizesChanged {
                    border_width,
                    title_height,
        ...
        let damage = or_chain!(_______________________________________________)
            || border
            || separator
            || or_chain!();
        if damage {
...
fn compute_theme(theme: &Theme, slf: Option<&ToplevelData>) -> ContainerTheme {
    define_ident_mut!(theme.@container_borders.get());
    define_ident_mut!(theme.@show_titles.get());
    define_ident_mut!(theme.colors.@border.get());
    define_ident_mut!(theme.colors.@separator.get());
    define_ident_mut!(theme.sizes.@border_width.val.get());
    define_ident_mut!(theme.sizes.@title_height.val.get());
    if let Some(theme) = slf.and_then(|d| d.theme(SelfTheme)) {
        write_ident_opt!(theme.@container_borders.get());
    ...
    define_ident!(Cell::new(@container_borders));
...
fn compute_child_theme(
    theme: &Theme,
    container_theme: &ContainerTheme,
    slf: Option<&ToplevelData>,
    cld: &ToplevelData,
) -> ContainerChildTheme {
    define_ident_mut!(theme.colors.@border.val.get());
    define_ident_mut!(theme.colors.@focused_title_text.val.get());
    define_ident_mut!(theme.colors.@focused_inactive_title_text.val.get());
    ...
    define_ident_mut!(theme.colors.@focused_inactive_title_background.val.get());
    ...
    for theme in [
        slf.and_then(|d| d.theme(SelfTheme)), //
        cld.theme(ParentTheme),
    ] {
        if let Some(theme) = theme {
            write_ident_opt!(theme.colors.@border.get());
            write_ident_opt!(theme.colors.@focused_title_text.get());
            write_ident_opt!(theme.colors.@focused_inactive_title_text.get());
            ...
            write_ident_opt!(theme.colors.@focused_inactive_title_background.get());
    ...
    define_ident!(Cell::new(@focused_inactive_title_text));
    ...
    define_ident!(Cell::new(@focused_inactive_title_background));
...
impl Transactionable for ContainerNode {
    ...
    fn apply(self: &Rc<Self>, op: Self::T) {
        ...
        match op {
            ...
            ContainerTransactionOp::ThemeOp(v) => {
