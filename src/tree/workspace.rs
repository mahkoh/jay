...
impl WorkspaceNode {
    ...
    pub fn set_container(self: &Rc<Self>, tt: &TreeTransaction, container: &Rc<ContainerNode>) {
        ...
        container.clone().tl_request_config(tt, pos.size2());
        container.clone().tl_set_mapped_position(&pos);
    ...
    pub fn change_extents(&self, tt: &TreeTransaction, rect: &Rect) {
        ...
        if let Some(c) = self.container.get() {
            c.tl_request_config(tt, rect.size2());
        }
    }
    ...
    pub fn set_fullscreen_node(self: &Rc<Self>, tt: &TreeTransaction, node: &Rc<dyn ToplevelNode>) {
        ...
        node.clone().tl_request_config(tt, pos.size2());
        tt.add_op(WorkspaceTreeOp {
            ws: self.clone(),
            kind: WorkspaceTreeOpKind::FullscreenNodeSet(node.clone()),
        });
    }
    ...
    pub fn remove_fullscreen_node(self: &Rc<Self>, tt: &TreeTransaction) {
        if let Some(node) = self.pending_fullscreen.take() {
            ...
            tt.add_op(WorkspaceTreeOp {
                ws: self.clone(),
                kind: WorkspaceTreeOpKind::FullscreenNodeUnset,
            });
        }
    }
