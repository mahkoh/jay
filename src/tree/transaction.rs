...
impl Default for TreeTransactions {
    fn default() -> Self {
        Self {
            timeout_ns: Cell::new(100_000_000),
...
impl Drop for TreeBarrier {
    fn drop(&mut self) {
        ...
        // b.pending_barrier_bts.remove(&self.id);
...
impl TreeBlockerInner {
    fn unblock(self: &Rc<Self>, timed_out: bool) {
        ...
        if timed_out {
            // let mut bt = self.bt.take().unwrap();
            // bt.resolve();
            log::warn!("timeout!! {:?}", self.serial.get());
            // for mut bt in self.pending_barrier_bts.lock().drain_values() {
            //     bt.resolve();
            //     log::warn!("barrier: {:?}", bt);
            // }
        }
        // self.pending_barrier_bts.clear();
...
impl TreeTransaction<'_> {
    ...
    pub fn barrier(&self) -> TreeBarrier {
        // let id = self.blocker.inner.barrier_ids.fetch_add(1);
        self.blocker.inner.pending_barriers.fetch_add(1);
        // self.blocker
        //     .inner
        //     .pending_barrier_bts
        //     .set(id, Backtrace::new_unresolved());
        TreeBarrier {
            // id,
...
impl Drop for TreeTransaction<'_> {
    fn drop(&mut self) {
        ...
        if b.pending_barriers.get() == 0 {
            ...
            // );
            // log::info!("push {}", o.inner.start_ns.get());
...
pub async fn handle_tree_blocker_unblocked(state: Rc<State>) {
    ...
    loop {
        ...
        while let Some(blocker) = tb.unblock_queue.try_pop() {
            // let d =Duration::from_nanos(state.now_nsec() - blocker.start_ns.get());
            // let blocker = tb.unblock_queue.pop().await;
            let timed_out = blocker.timed_out.get();
            // if d.as_millis() >= 100 {
            //     log::info!("applying (to = {timed_out}) transaction after {d:?}");
            // }
    ...
    loop {
        ...
        let _timeout = state.eng.spawn("tree blocker timeout impl", async move {
            ...
            loop {
                ...
                // if timeout_ns < prev_timeout_ns {
                //     log::info!("went backwards");
                // }
