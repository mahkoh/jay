...
impl Drop for ConfigureGroup {
    fn drop(&mut self) {
        ...
        for member in &*self.inner.members.borrow() {
            // log::info!("push to {:?}", &self.inner.groups.ready as *const _);
...
impl ConfigureGroup {
        ...
        C: Configurable,
    {
        ...
        let requests = &mut *d.core.requests.borrow_mut();
        if !d.core.idle.get() || requests.len() > 0 {
            self.inner.num_not_ready.fetch_add(1);
        }
        requests.push_back(self.inner.clone());
        d.requests.borrow_mut().push_back(data);
    }
}
