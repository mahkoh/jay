...
impl ConfigProxyHandler {
    ...
    fn handle_set_default_mono_style(&self, style: JcMonoStyle) {
        self.state.set_default_mono_style(style.into());
    }
    ...
    fn handle_set_seat_mono_style(&self, seat: Seat, style: JcMonoStyle) -> Result<(), CphError> {
        let seat = self.get_seat(seat)?;
        seat.set_mono_style(style.into());
        Ok(())
    }
    ...
    fn ensure_window_matcher_properties(
        &self,
        matcher: WindowMatcher,
    ...
    fn handle_request_(self: &Rc<Self>, msg: &[u8]) -> Result<(), CphError> {
        ...
        match request {
            ...
            ClientMessage::SetDefaultMonoStyle { style } => {
                self.handle_set_default_mono_style(style)
            }
